...
def cheese_disease_dataset():
    ...
    src_filename = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "cheeseDisease.train.txt"
    )
...
def test_np_model(X_sequence):
    ...
    mod = RNNClassifier(vocab=vocab, embedding=embedding, hidden_dim=20, max_iter=100)
    mod.fit(X_train, y_train)
    mod.predict(X_test)
    mod.predict_proba(X_test)
...
def test_cheese_disease(cheese_disease_dataset):
    ...
    mod = TorchRNNClassifier(vocab=vocab, embed_dim=50, hidden_dim=50, max_iter=200)
    mod.fit(X_train, y_train)
...
def test_simple_example_params(X_sequence, param, expected):
    ...
    if param == "use_embedding" and expected == False:
        embedding = np.random.uniform(low=-1.0, high=1.0, size=(len(vocab), 60))
...
@pytest.mark.parametrize(
    "param, expected", [["hidden_dim", 10], ["eta", 1.0], ["max_iter", 100]]
)
...
@pytest.mark.parametrize("with_y, expected", [[True, 3], [False, 2]])
...
def test_pretrained_embedding(X_sequence):
    ...
    mod = TorchRNNClassifier(
        vocab, max_iter=1, embedding=embedding, freeze_embedding=True
    )
    mod.fit(X_train, y_train)
...
@pytest.mark.parametrize("freeze, outcome", [[True, True], [False, False]])
def test_embedding_update_control(X_sequence, freeze, outcome):
    X_train, X_test, y_train, y_test, vocab = X_sequence
    embed_dim = 5
    embedding = np.ones((len(vocab), embed_dim))
    mod = TorchRNNClassifier(
        vocab, max_iter=10, embedding=embedding, freeze_embedding=freeze
    )
    mod.fit(X_train, y_train)
    graph_emb = mod.model.rnn.embedding.weight.detach().cpu().numpy()
...
@pytest.mark.parametrize("model_class", [TorchRNNClassifier, RNNClassifier])
