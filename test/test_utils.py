...
@pytest.mark.parametrize(
    "arg, expected",
    [[np.array([0.0, 0.25, 0.75]), np.array([0.22721977, 0.29175596, 0.48102426])]],
)
...
@pytest.mark.parametrize(
    "arg, expected", [[-1, 0], [np.array([-1.0, 1.0]), np.array([0.0, 0.0])]]
)
...
@pytest.mark.parametrize(
    "arg, expected",
    [[np.array([[1.0, 0.0], [0.0, 1.0]]), np.array([[0.0, 0.0], [0.0, 0.0]])]],
)
...
@pytest.mark.parametrize(
    "X, n_words, mincount, expected",
    [
        [[["a", "b", "c"], ["b", "c", "d"]], None, 1, ["$UNK", "a", "b", "c", "d"]],
        [[["a", "b", "c"], ["b", "c", "d"]], 2, 1, ["$UNK", "b", "c"]],
        [[], 2, 1, ["$UNK"]],
        [[["a", "b", "b"], ["b", "c", "a"]], None, 3, ["$UNK", "b"]],
        [[["b", "b", "b"], ["b", "a", "a", "c"]], 2, 3, ["$UNK", "b"]],
    ],
)
...
@pytest.mark.parametrize(
    "lookup, vocab, required_tokens, expected_shape",
    [
        [{"a": [1, 2]}, ["a", "b"], ["$UNK"], (3, 2)],
...
def test_create_pretrained_embedding(lookup, vocab, required_tokens, expected_shape):
    result, new_vocab = utils.create_pretrained_embedding(
        lookup, vocab, required_tokens
    )
